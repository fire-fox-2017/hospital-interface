...
class Hospital {
  ...
  menu() {
    ...
    if (this.sess[0].position == 'Admin') {
      ...
      console.log('-- remove_employee <record_id>');
      console.log('-- list_patients');
      console.log('-- view_patient <record_id>');
      console.log('-- add_record');
      console.log('-- remove_patient <record_id>');
      console.log('-- Logout');
    }
    ...
    rl.question('\nWhat would you like to do?\noptions : ', (answer) => {
      switch (answer.split(' ')[0]) {
        ...
        case 'remove_employee':
          this.remove_employee(answer.split(' ')[1])
          break
        case 'list_patients':
          this.list_patients()
          break
        ...
        case 'add_record':
          ...
          break
        case 'remove_patient':
          this.remove_patient(answer.split(' ')[1])
          break
        case 'Logout':
          this.logout()
          break
  ...
  list_employee() {
    if (this.employees.length == 0) {
      ...
      console.log('\n================== List Employees ==================')
      var table = new Table({
        head: ['ID', 'Name', 'Position', 'Username'],
        colWidths: [5, 20, 20, 20]
      });
      for (let i = 0; i < this.employees.length; i++) {
        table.push(
          [`${i+1}`, `${this.employees[i].name}`, `${this.employees[i].position}`, `${this.employees[i].username}`]
        )
      }
  ...
  add_employee() {
    ...
    rl.question('\nMasukan nama,position,username,password, (ex: dudi,doctor,admin,1234) : ', (answer) => {
      if (answer != '') {
        ...
        fs.writeFileSync('employees.json', JSON.stringify(this.employees))
        this.list_employee2()
        console.log('\n')
        this.menu()
      } else {
  ...
  add_record() {
    ...
    rl.question('\nMasukkan nama & diagnosis, (ex: dodit:flu) : ', (answer) => {
      if (answer !== '') {
        ...
        console.log('Insert Success !')
        fs.writeFileSync('patients.json', JSON.stringify(this.patients), 'utf8');
        this.listPasien2()
        console.log('\n')
        this.menu()
      } else {
  ...
  remove_employee(id) {
    ...
    for (let i = 0; i < this.employees.length; i++) {
      if (listToDelete.indexOf(this.employees[i].id) !== -1) {
        this.employees.splice(i, 1); // start, jmlHapus
      }
    }

    fs.writeFileSync('employees.json', JSON.stringify(this.employees), 'utf8');
    this.list_employee2()
    console.log('\n')
...
let dataEmployee = JSON.parse(fs.readFileSync('employees.json', 'utf-8'))
// console.log(dataEmployee);
let dataPatient = JSON.parse(fs.readFileSync('patients.json', 'utf-8'))
let hospitalFire = new Hospital("Hospital Fox", dataEmployee, dataPatient, "Pondok Indah")
hospitalFire.start()