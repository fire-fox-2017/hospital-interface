...
class Hospital {
  ...
  password(){
    rl.question("Please enter your password :",(answer) => {
      if(this.tmp_login[0].password == answer){
        this.start(this.tmp_login[0].name,this.tmp_login[0].position)
      }else{
...
class Patient {
  ...
  addPatient(){
  rl.question('Entry this data: (ex: 1,Zul,Diare) => ',(input) => {
    if(input != ''){
      input = input.split(',')
...
let objEmployee = JSON.parse(fs.readFileSync('employee.json','utf-8'));
let objPatient = JSON.parse(fs.readFileSync('patient.json','utf-8'));
let employeeSys = new Employee();
let patientSys = new Patient();
let hospitalSys = new Hospital("Hacktiv8","Jakarta Selatan",objEmployee,objPatient);
